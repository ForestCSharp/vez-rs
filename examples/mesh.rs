...
fn main() {
    ...
    let obj_file = fs::read_to_string("examples/assets/monkey.obj")
    .expect("failed to open monkey.obj");
    ...
    let vertices : Vec<Vertex> = positions_iter.zip(normals_iter).map(|(pos, norm)| Vertex {
    ...
    let triangles : Vec<[usize;3]> = first_object.geometry
        .iter().map(|g| g.shapes.iter().filter_map(|s| {
            match s.primitive {
    ...
    let indices : Vec<u32> = triangles.iter().flat_map(|tri| tri.iter()).copied().map(|i| i as u32).collect();

    let index_buffer = logical_device.create_buffer(
    ...
    let uniform_buffer = logical_device.create_buffer(
    ...
    let record_command_buffer = |frame_data : &FrameData, width : u32, height : u32| {
        command_buffer.begin_recording(CommandBufferUsage::SIMULTANEOUS_USE, |command_recorder| {
            ...
            command_recorder.begin_render_pass(&frame_data.framebuffer, &[
                VezAttachmentInfo {
...
impl FrameData {
    fn new(instance : &Instance, logical_device : &LogicalDevice, window : &glfw::Window) -> FrameData {
        ...
        let depth_image = logical_device.create_image(
            width,
            height,
            depth_format,
            MemoryUsage::GPU_ONLY,
        ...
        let depth_image_view = logical_device
            .create_image_view(&depth_image, VkImageViewType::VK_IMAGE_VIEW_TYPE_2D,depth_format)
            .unwrap();
