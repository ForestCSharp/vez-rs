...
fn main() {
    ...
    let compute_shader_module = logical_device.create_shader_module(
        ShaderStage::COMPUTE,
        ShaderCode::GLSL(compute_shader_source.to_string()),
        "main"
    ...
    let compute_pipeline = logical_device
        .create_compute_pipeline(&compute_shader_module)
    ...
    //TODO: what happens if you don't specify this?
    let vertex_input_format = logical_device.create_vertex_input_format(
        &[
    ...
    ).expect("failed to create vertex input format");
    ...
    let record_command_buffer = |frame_data : &FrameData, width : u32, height : u32| {
        command_buffer.begin_recording(CommandBufferUsage::SIMULTANEOUS_USE, |command_recorder| {
            ...
            command_recorder.begin_render_pass(&frame_data.framebuffer, &[VezAttachmentInfo {
                ...
                clearValue : ClearValue::Color(ClearColor::Float([0.39, 0.58, 0.93, 1.0])).into(),
            }], || {
                command_recorder.bind_pipeline(&graphics_pipeline);
                command_recorder.bind_vertex_buffer(0, &vertex_buffer, 0);
                command_recorder.set_vertex_input_format(&vertex_input_format);
    ...
    logical_device.destroy_vertex_input_format(vertex_input_format);
    logical_device.destroy_pipeline(compute_pipeline);
    ...
    logical_device.destroy_shader_module(compute_shader_module);
