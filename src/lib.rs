...
impl InstanceBuilder {
    ...
    pub fn enumerate_extension_properties(
        ...
        unsafe {
            ...
            vezEnumerateInstanceExtensionProperties(
                p_layer_name,
                &mut property_count,
                std::ptr::null_mut(),
            );
            ...
            if raw_properties.len() > 0 {
                return raw_properties
                    .iter_mut()
                    .map(|raw| ExtensionProperties {
                        name: CStr::from_ptr(raw.extensionName.as_ptr())
                            .to_owned()
                            .into_string()
    ...
    pub fn enumerate_layer_properties(&self) -> Vec<LayerProperties> {
        unsafe {
            ...
            if raw_properties.len() > 0 {
                return raw_properties
                    .iter_mut()
                    .map(|raw| LayerProperties {
                        name: CStr::from_ptr(raw.layerName.as_ptr())
                            .to_owned()
                            .into_string()
                            .expect("failed to convert string"),
                        spec_version: raw.specVersion,
                        implementation_version: raw.implementationVersion,
                        description: CStr::from_ptr(raw.description.as_ptr())
                            .to_owned()
                            .into_string()
                            .expect("failed to convert string"),
                    })
...
impl Instance {
    ...
    pub fn enumerate_physical_devices(&self) -> Result<Vec<PhysicalDevice>, VkResult> {
        unsafe {
            ...
            let result = vezEnumeratePhysicalDevices(
                self.raw,
            ...
            let mut raw_physical_devices =
                vec![std::ptr::null_mut(); physical_device_count as usize];
            let result = vezEnumeratePhysicalDevices(
                self.raw,
                &mut physical_device_count,
                raw_physical_devices.as_mut_ptr(),
            );
            if result != VkResult::VK_SUCCESS {
                return Err(result);
            }
            ...
            if raw_physical_devices.len() > 0 {
                return Ok(raw_physical_devices
                    .iter_mut()
                    .map(|raw| PhysicalDevice { raw: *raw })
                    .collect());
            }
        }
...
impl LogicalDeviceBuilder {
    ...
    pub fn build(&mut self) -> Result<LogicalDevice, VkResult> {
        ...
        unsafe {
            ...
            let result = vezCreateDevice(
                self.physical_device,
                &create_info,
                logical_device.as_mut_ptr(),
            );
...
impl LogicalDevice {
    ...
    fn get_typed_queue_helper(
        &self,
        get_queue_function: unsafe extern "C" fn(VkDevice, u32, *mut VkQueue),
        queue_index: u32,
    ) -> Queue {
        let queue = unsafe {
            let mut queue = MaybeUninit::zeroed();
    ...
    pub fn create_swapchain(
        ...
        Ok(Swapchain { raw: raw_swapchain })
    }
    ...
    pub fn create_shader_module(
        ...
        let (code_size, glsl_source, code) = match code {
            ShaderCode::GLSL(glsl) => (
                glsl.len(),
                Some(CString::new(glsl.clone()).expect("failed to create CString")),
                None,
            ),
        ...
        let raw = unsafe {
            ...
            let result =
                vezCreateShaderModule(self.raw, &create_info, raw_shader_module.as_mut_ptr());
    ...
    pub fn create_graphics_pipeline<'a>(
        &self,
        shader_modules: &[&'a ShaderModule],
    ) -> Result<Pipeline<'a>, VkResult> {
        let shader_stage_create_infos: Vec<VezPipelineShaderStageCreateInfo> = shader_modules
            .iter()
            .map(|module| {
        ...
        let raw = unsafe {
            ...
            let result = vezCreateGraphicsPipeline(
                self.raw,
    ...
    pub fn create_compute_pipeline<'a>(
        ...
        let raw = unsafe {
            let mut pipeline = MaybeUninit::zeroed();
            let result = vezCreateComputePipeline(
                self.raw,
    ...
    pub fn create_vertex_input_format(
        ...
        let raw = unsafe {
            ...
            let result = vezCreateVertexInputFormat(
                self.raw,
                &create_info,
                vertex_input_format.as_mut_ptr(),
            );
            if result != VkResult::VK_SUCCESS {
                return Err(result);
            }
    ...
    pub fn create_buffer(
        ...
        let raw = unsafe {
            ...
            let result =
                vezCreateBuffer(self.raw, memory_flags.0, &create_info, buffer.as_mut_ptr());
            if result != VkResult::VK_SUCCESS {
                return Err(result);
            }
            buffer.assume_init()
    ...
    pub fn create_buffer_view(
        ...
        buffer: &Buffer,
        format: VkFormat,
        range: std::ops::Range<VkDeviceSize>,
    ...
    pub fn create_image_view(
        ...
        let create_info = VezImageViewCreateInfo {
            ...
            subresourceRange: VezImageSubresourceRange {
                //TODO:
                baseMipLevel: 0,
                levelCount: 1,
                baseArrayLayer: 0,
    ...
    pub fn create_framebuffer(
        ...
        let p_attachments: Vec<VkImageView> = attachments
            .iter()
            .map(|attachment| attachment.raw)
            .collect();
    ...
    pub fn allocate_command_buffers(
        ...
        let command_buffers = unsafe {
            ...
            let result =
                vezAllocateCommandBuffers(self.raw, &allocate_info, &mut p_command_buffers);
            if result != VkResult::VK_SUCCESS {
                return Err(result);
            }
    ...
    pub fn free_command_buffers(&self, command_buffers: &[CommandBuffer]) {
        let raw_command_buffers: Vec<VkCommandBuffer> = command_buffers
            .iter()
            .map(|command_buffer| command_buffer.raw)
            .collect();
        unsafe {
            vezFreeCommandBuffers(
                self.raw,
                command_buffers.len() as u32,
                raw_command_buffers.as_ptr(),
            );
        }
    }
}
...
impl Swapchain {
    ...
    pub fn set_vsync_enabled(&self, enabled: bool) -> VkResult {
        unsafe { vezSwapchainSetVSync(self.raw, enabled as u32) }
    }
}
...
impl Queue {
    ...
    pub fn submit(&self, queue_submissions: &[QueueSubmission]) -> Result<Fence, VkResult> {
        ...
        let mut raw_submissions_data: Vec<RawSubmissionData> = queue_submissions
            .iter()
            .map(|submission| {
                let raw_command_buffers: Vec<VkCommandBuffer> = submission
                    .command_buffers
                    .iter()
                    .map(|command_buffer| command_buffer.raw)
                    .collect();
                let mut raw_signal_semaphores: Vec<VkSemaphore> = raw_command_buffers
                    .iter()
        ...
        let raw_submit_infos: Vec<VezSubmitInfo> = raw_submissions_data
            .iter_mut()
            .map(|submit_data| {
                VezSubmitInfo {
                    pNext: std::ptr::null(),
        ...
        let raw = unsafe {
            ...
            let result = vezQueueSubmit(
                self.raw,
                raw_submit_infos.len() as u32,
                raw_submit_infos.as_ptr(),
                fence.as_mut_ptr(),
            );
            if result != VkResult::VK_SUCCESS {
                return Err(result);
    ...
    pub fn present(&self, swapchains_and_images: &[(&Swapchain, &Image)]) -> VkResult {
        let raw_swapchains: Vec<VezSwapchain> = swapchains_and_images
            .iter()
            .map(|(swapchain, _)| swapchain.raw)
            .collect();
        let raw_images: Vec<VkImage> = swapchains_and_images
            .iter()
            .map(|(_, image)| image.raw)
            .collect();

        let mut raw_signal_semaphores: Vec<VkSemaphore> = swapchains_and_images
            .iter()
            .map(|_| std::ptr::null_mut())
            .collect();
        let mut raw_results: Vec<VkResult> = swapchains_and_images
            .iter()
            .map(|_| VkResult::VK_SUCCESS)
            .collect();
    ...
    pub fn wait_idle(&self) -> VkResult {
        unsafe { vkQueueWaitIdle(self.raw) }
    }
}
...
impl CommandBuffer {
    pub fn begin_recording<F: Fn(&CommandRecorder)>(
        &self,
...
impl<'a> CommandRecorder<'a> {
    pub fn begin_render_pass<F: Fn()>(
        &self,
        framebuffer: &Framebuffer,
        attachments: &[VezAttachmentInfo],
        render_pass_function: F,
    ) {
        let begin_info = VezRenderPassBeginInfo {
            pNext: std::ptr::null(),
            framebuffer: framebuffer.raw,
            attachmentCount: attachments.len() as u32,
            pAttachments: if attachments.len() <= 0 {
                std::ptr::null()
            } else {
                attachments.as_ptr()
            },
    ...
    pub fn push_constants<T>(&self, offset: u32, values: &[T]) {
        unsafe {
            vezCmdPushConstants(
                offset,
                (std::mem::size_of::<T>() * values.len()) as _,
                values.as_ptr() as _,
            );
        }
    }
    ...
    pub fn bind_vertex_buffers(
        &self,
        first_index: u32,
        vertex_buffers: &[(&Buffer, VkDeviceSize)],
    ) {
        unsafe {
            let buffers: Vec<VkBuffer> = vertex_buffers
                .iter()
                .map(|(buffer, _)| buffer.raw)
                .collect();
            let offsets: Vec<VkDeviceSize> = vertex_buffers
                .iter()
                .map(|(_, offset)| offset)
                .cloned()
                .collect();
    ...
    pub fn set_input_assembly_state(&self, state: &VezInputAssemblyState) {
        //TODO: rustified structs
        unsafe {
            vezCmdSetInputAssemblyState(state);
        }
    }

    pub fn set_rasterization_state(&self, state: &VezRasterizationState) {
        //TODO: rustified structs
        unsafe {
    ...
    pub fn set_multisample_state(&self, state: &VezMultisampleState) {
        //TODO: rustified structs
        unsafe {
    ...
    pub fn set_depth_stencil_state(&self, state: &VezDepthStencilState) {
        //TODO: rustified structs
        unsafe {
    ...
    pub fn set_color_blend_state(&self, state: &VezColorBlendState) {
        //TODO: rustified structs
        unsafe {
    ...
    pub fn fill_buffer(
        &self,
        dst: &Buffer,
        fill_region: std::ops::Range<VkDeviceSize>,
        value: u32,
    ) {
        unsafe {
            vezCmdFillBuffer(
                dst.raw,
                fill_region.start,
                fill_region.end - fill_region.start,
                value,
            );
        }
    }
    ...
    pub fn clear_color_image(
        ...
        ranges: &[VezImageSubresourceRange],
    ) {
        unsafe {
            vezCmdClearColorImage(
                image.raw,
                &clear_value.into(),
                ranges.len() as _,
                ranges.as_ptr(),
...
impl MemoryUsage {
    ...
    pub const DEDICATED_ALLOCATION: MemoryUsage =
        MemoryUsage(VezMemoryFlagsBits::VEZ_MEMORY_DEDICATED_ALLOCATION as _);
    pub const NO_ALLOCATION: MemoryUsage =
        MemoryUsage(VezMemoryFlagsBits::VEZ_MEMORY_NO_ALLOCATION as _);
}
...
impl CommandBufferUsage {
    pub const ONE_TIME_SUBMIT: CommandBufferUsage = CommandBufferUsage(
        VkCommandBufferUsageFlagBits::VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT as _,
    );
    pub const RENDER_PASS_CONTINUE: CommandBufferUsage = CommandBufferUsage(
        VkCommandBufferUsageFlagBits::VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT as _,
    );
    pub const SIMULTANEOUS_USE: CommandBufferUsage = CommandBufferUsage(
        VkCommandBufferUsageFlagBits::VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT as _,
    );
}
...
impl BufferUsage {
    pub const TRANSFER_SRC: BufferUsage =
        BufferUsage(VkBufferUsageFlagBits::VK_BUFFER_USAGE_TRANSFER_SRC_BIT as _);
    pub const TRANSFER_DST: BufferUsage =
        BufferUsage(VkBufferUsageFlagBits::VK_BUFFER_USAGE_TRANSFER_DST_BIT as _);
    pub const UNIFORM_TEXEL_BUFFER: BufferUsage =
        BufferUsage(VkBufferUsageFlagBits::VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT as _);
    pub const STORAGE_TEXEL_BUFFER: BufferUsage =
        BufferUsage(VkBufferUsageFlagBits::VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT as _);
    pub const UNIFORM_BUFFER: BufferUsage =
        BufferUsage(VkBufferUsageFlagBits::VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT as _);
    pub const STORAGE_BUFFER: BufferUsage =
        BufferUsage(VkBufferUsageFlagBits::VK_BUFFER_USAGE_STORAGE_BUFFER_BIT as _);
    pub const INDEX_BUFFER: BufferUsage =
        BufferUsage(VkBufferUsageFlagBits::VK_BUFFER_USAGE_INDEX_BUFFER_BIT as _);
    pub const VERTEX_BUFFER: BufferUsage =
        BufferUsage(VkBufferUsageFlagBits::VK_BUFFER_USAGE_VERTEX_BUFFER_BIT as _);
    pub const INDIRECT_BUFFER: BufferUsage =
        BufferUsage(VkBufferUsageFlagBits::VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT as _);
}
...
impl ImageUsage {
    pub const TRANSFER_SRC: ImageUsage =
        ImageUsage(VkImageUsageFlagBits::VK_IMAGE_USAGE_TRANSFER_SRC_BIT as _);
    pub const TRANSFER_DST: ImageUsage =
        ImageUsage(VkImageUsageFlagBits::VK_IMAGE_USAGE_TRANSFER_DST_BIT as _);
    pub const SAMPLED: ImageUsage =
        ImageUsage(VkImageUsageFlagBits::VK_IMAGE_USAGE_SAMPLED_BIT as _);
    pub const STORAGE: ImageUsage =
        ImageUsage(VkImageUsageFlagBits::VK_IMAGE_USAGE_STORAGE_BIT as _);
    pub const COLOR_ATTACHMENT: ImageUsage =
        ImageUsage(VkImageUsageFlagBits::VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT as _);
    pub const DEPTH_STENCIL_ATTACHMENT: ImageUsage =
        ImageUsage(VkImageUsageFlagBits::VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT as _);
    pub const TRANSIENT_ATTACHMENT: ImageUsage =
        ImageUsage(VkImageUsageFlagBits::VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT as _);
    pub const INPUT_ATTACHMENT: ImageUsage =
        ImageUsage(VkImageUsageFlagBits::VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT as _);
}
...
impl PipelineStage {
    pub const TOP_OF_PIPE: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT as _);
    pub const DRAW_INDIRECT: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_DRAW_INDIRECT_BIT as _);
    pub const VERTEX_INPUT: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_VERTEX_INPUT_BIT as _);
    pub const VERTEX_SHADER: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_VERTEX_SHADER_BIT as _);
    pub const TESSELLATION_CONTROL_SHADER: PipelineStage = PipelineStage(
        VkPipelineStageFlagBits::VK_PIPELINE_STAGE_TESSELLATION_CONTROL_SHADER_BIT as _,
    );
    pub const TESSELLATION_EVALUATION_SHADER: PipelineStage = PipelineStage(
        VkPipelineStageFlagBits::VK_PIPELINE_STAGE_TESSELLATION_EVALUATION_SHADER_BIT as _,
    );
    pub const GEOMETRY_SHADER: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_GEOMETRY_SHADER_BIT as _);
    pub const FRAGMENT_SHADER: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT as _);
    pub const EARLY_FRAGMENT_TESTS: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT as _);
    pub const LATE_FRAGMENT_TESTS: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT as _);
    pub const COLOR_ATTACHMENT_OUTPUT: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT as _);
    pub const COMPUTE_SHADER: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT as _);
    pub const TRANSFER: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_TRANSFER_BIT as _);
    pub const BOTTOM_OF_PIPE: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT as _);
    pub const HOST: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_HOST_BIT as _);
    pub const ALL_GRAPHICS: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT as _);
    pub const ALL_COMMANDS: PipelineStage =
        PipelineStage(VkPipelineStageFlagBits::VK_PIPELINE_STAGE_ALL_COMMANDS_BIT as _);
}
...
impl StencilFace {
    pub const FRONT: StencilFace =
        StencilFace(VkStencilFaceFlagBits::VK_STENCIL_FACE_FRONT_BIT as _);
    pub const BACK: StencilFace = StencilFace(VkStencilFaceFlagBits::VK_STENCIL_FACE_BACK_BIT as _);
    pub const BOTH: StencilFace =
        StencilFace(VkStencilFaceFlagBits::VK_STENCIL_FRONT_AND_BACK as _);
}
...
impl ShaderStage {
    ...
    pub const ALL_GRAPHICS: ShaderStage =
        ShaderStage(VkShaderStageFlagBits::VK_SHADER_STAGE_ALL_GRAPHICS);
...
impl From<ClearColor> for VkClearColorValue {
    fn from(clear_color: ClearColor) -> Self {
        match clear_color {
            ClearColor::Float(float_array) => VkClearColorValue {
                float32: float_array,
            },
            ClearColor::Int(int_array) => VkClearColorValue { int32: int_array },
            ClearColor::UInt(unsigned_int_array) => VkClearColorValue {
                uint32: unsigned_int_array,
            },
        }
    }
}
...
impl From<ClearValue> for VkClearValue {
    fn from(clear_value: ClearValue) -> Self {
        match clear_value {
            ClearValue::Color(clear_color) => VkClearValue {
                color: clear_color.into(),
            },
            ClearValue::DepthStencil(clear_depth_stencil) => VkClearValue {
